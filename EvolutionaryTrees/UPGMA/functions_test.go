...
var tests = []fme_TestingPair{
  {"Tests/matrix1.txt", 2.0},
  {"Tests/matrix5.txt", 2.0},
  {"Tests/matrix6.txt", 8.0},
  {"Tests/matrix7.txt", 1.0},
  {"Tests/matrix8.txt", 2.0},
  {"Tests/matrix9.txt", 2.0},
  {"Tests/matrix10.txt", 2.0}}

func TestFindMinElement(t *testing.T) {
  for _, test := range tests {

    mtx, _ := Io.ReadMatrixFromFile(test.matrixPath)
    _, _, val := FindMinElement(mtx)
...
func createTree1() Datatypes.Tree {
  T := make([]*Datatypes.Node, 7)

  var NODE0 Datatypes.Node
  var NODE1 Datatypes.Node
  var NODE2 Datatypes.Node
  var NODE3 Datatypes.Node
  var NODE4 Datatypes.Node
  var NODE5 Datatypes.Node
  var NODE6 Datatypes.Node
...
func createTree2() Datatypes.Tree {
  T := make([]*Datatypes.Node, 7)
...
func createTree3() Datatypes.Tree {
  T := make([]*Datatypes.Node, 15)
  ...
  var NODE5 Datatypes.Node
  var NODE6 Datatypes.Node
  var NODE7 Datatypes.Node
  var NODE8 Datatypes.Node
  var NODE9 Datatypes.Node
  var NODE10 Datatypes.Node
  var NODE11 Datatypes.Node
  var NODE12 Datatypes.Node
  var NODE13 Datatypes.Node
  var NODE14 Datatypes.Node
...
type arc_TestingPair struct {
  tree  Datatypes.Tree
  mtx   Datatypes.DistanceMatrix
  cltr  []*Datatypes.Node
  row   int
  col   int
  ans   Datatypes.DistanceMatrix
}
...
var arc_tests = []arc_TestingPair{
  {TREE2,
    [][]float64{[]float64{0, 16},
      []float64{16, 0}},
    []*Datatypes.Node{TREE2[1], TREE1[5]},
    0,
    1,
  ...
  {TREE1,
    [][]float64{[]float64{0, 1, 3},
      []float64{1, 0, 3},
      []float64{3, 3, 0}},
    []*Datatypes.Node{TREE1[0], TREE1[1], TREE1[4]},
    0,
    1,
  ...
  {TREE1,
    [][]float64{[]float64{0, 3, 3, 1},
      ...
      []float64{3, 1, 0, 3},
      []float64{1, 3, 3, 0}},
    []*Datatypes.Node{TREE1[0], TREE1[1], TREE1[2], TREE1[3]},
    0,
    3,
  ...
  {TREE2,
    [][]float64{[]float64{0, 20, 9, 11},
      ...
      []float64{9, 17, 0, 8},
      []float64{11, 11, 8, 0}},
    []*Datatypes.Node{TREE2[0], TREE2[1], TREE2[2], TREE2[3]},
    2,
    3,
  ...
  {TREE3,
    ...
    []*Datatypes.Node{TREE3[0], TREE3[1], TREE3[2], TREE3[5], TREE3[6], TREE3[7], TREE3[8]},
    0,
    1,
...
func TestDelRowCol(t *testing.T) {
  for _, test := range drc_tests {

    mtx, _ := Io.ReadMatrixFromFile(test.matrixPath)
    mtxCopy, _ := Io.ReadMatrixFromFile(test.matrixPath)
    val := DelRowCol(mtx, test.r, test.c)
...
func MtxEq(mtx1, mtx2 [][]float64) bool {
    if (len(mtx1) != len(mtx2)) {return false}
    for i := range mtx1 {
        if (len(mtx1[i]) != len(mtx2[i])) {return false}
        for j := range mtx1[i] {
...
func createTree() Datatypes.Tree {
  T := make([]*Datatypes.Node, 9)
...
func TestUPGMA(t *testing.T) {
  for _, test := range upgma_tests {
    ...
    mtx, labels := Io.ReadMatrixFromFile(test.matrixPath)
    mtxCopy, _ := Io.ReadMatrixFromFile(test.matrixPath)
    T := UPGMA(mtx, labels)
    strT := Io.ToNewick(T)
